...
export default function SoundsLikeTheRadio(props) {
  const { nodes, materials } = useGLTF("./buttons/right/soundsLikeTheRadio.glb");
  const button = useRef()
  const trim = useRef()
  const text = useRef()
  ...
  return (
    <group {...props} dispose={null} scale={[ .9, .9, .9 ]} position={ props.position } rotation={ props.rotation }>
...
useGLTF.preload("./buttons/right/soundsLikeTheRadio.glb");
